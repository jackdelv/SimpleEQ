...
void LookAndFeel::drawToggleButton(juce::Graphics &g, juce::ToggleButton &toggleButton, bool shouldDrawButtonAsHighlighted, bool shouldDrawButtonAsDown)
{
    ...
    if (auto* pb = dynamic_cast<PowerButton*>(&toggleButton))
    {
        ...
        auto color = toggleButton.getToggleState() ? Colours::darkred : Colour(0u, 172u, 1u);
...
void ResponseCurveComponent::updateChain()
{
    ...
    auto lowCutCoefficients = makeLowCutFiter(chainSettings, audioProcessor.getSampleRate());
    auto highCutCoefficients = makeHighCutFiter(chainSettings, audioProcessor.getSampleRate());
...
AnalyzerEnabledButtonAttachment(audioProcessor.apvts, "Analyzer Enabled", analyzerEnabledButton)
{
    ...
    auto safePointer = juce::Component::SafePointer<SimpleEQAudioProcessorEditor> (this);
    peakBypassButton.onClick = [safePointer]()
    {
        if (auto* comp = safePointer.getComponent())
        {
    ...
    analyzerEnabledButton.onClick = [safePointer]()
    {
        if (auto* comp = safePointer.getComponent())
        {
...
//    analyzerEnabledArea.removeFromTop(2);
    ...
    float hRatio  = 25.5 / 100.f; // JUCE_LIVE_CONSTANT(33) / 100.f;
